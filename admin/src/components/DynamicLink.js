        ...
        const baseUrl = window.location.origin;
            ...
            if (currentStep === 1) {
                ...
                if (urlEncoding === 'encoded') {
                    ...
                    // For composite products, use the full URL generated by the backend
                    // Composite products use ?add-to-cart=ID&wccp_... format, not /checkout-link/
                    const compositeProduct = selectedProducts.find(p => p.type === 'composite' && (p.checkout_url || p.url));
                    
                    if (compositeProduct) {
                        // Return the full URL from the composite product
                        // Decode for display (convert %5B to [ and %5D to ])
                        const fullUrl = (compositeProduct.checkout_url || compositeProduct.url).replace(/%5B/g, '[').replace(/%5D/g, ']');
                        console.log('🔗 Final Generated Checkout URL:', fullUrl);
